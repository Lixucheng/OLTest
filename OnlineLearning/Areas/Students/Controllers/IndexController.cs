...
namespace OnlineLearning.Areas.Students.Controllers
{
    public class IndexController : BaseController
    {
        ...
        public ActionResult GetThisScore(int id)
        {
            ...
            var answer =
                Db.Answer.Count(e => e.studentId == studentid && e.TestId == id && !string.IsNullOrEmpty(e.Answer1));
            var all = Db.Test_question.Count(e => e.TestId == id);

            var scoreall = 0;
            Db.Test_question.Where(e => e.TestId == id)
                .ToList()
                .ForEach(e => { scoreall += Db.Question.Find(e.QuestionId).score; });
